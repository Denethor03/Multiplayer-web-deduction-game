...
def handle_join_lobby(data):
    ...
    join_room(room) # build in function that adds player sid to room
                    # sid is hidden and gotten from sender
    ...
    if player_count == REQUIRED_PLAYERS:
        ...
        for i, player in enumerate(players_in_room):
            player['team'] = 'Sentinels' if i < team_a_size else 'Heretics'
...
def handle_join_game(data):
    join_room(data['room'])
    send({'nick': 'System', 'text': f'{data["nick"]} has entered the game.'}, to=data['room'])
...
def handle_qr_scan(data):
    ...
    if location:
        send({'nick': 'System', 'text': f'Location "{location["name"]}" was visited.'}, to=room)
...
def handle_game_action(data):   
    ...
    response_text = f'[TES] {username} chose to "{action}" at {location_name}.'
